...
import { getFirestore, doc, setDoc, onSnapshot, collection, deleteDoc, writeBatch, updateDoc } from 'firebase/firestore';

// Definição manual de ícones SVG oficiais
const Icons = {
...
const App = () => {
  ...
  const allocationsCol = collection(db, 'artifacts', appId, 'public', 'data', 'allocations');
  ...
  useEffect(() => {
    ...
    const unsubDiscarded = onSnapshot(discardedOpsCol, (snap) => {
      const data = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setDiscardedOps(data.sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0)));
    });
    const unsubSet = onSnapshot(settingsDoc, (docSnap) => {
      if (docSnap.exists()) setLossValue(docSnap.data().lossValue || 200);
    });
    return () => { unsubAlloc(); unsubDiscarded(); unsubSet(); };
  }, [user]);
  ...
  const handleSequenciar = async () => {
    ...
    sorted.forEach((item, index, arr) => {
      ...
      batch.update(doc(allocationsCol, String(item.id)), { sequencia: currentSeq });
    });
    await batch.commit();
  };
  ...
  const handleJuncao = async () => {
    ...
    const batch = writeBatch(db);
    allocations.forEach(item => batch.delete(doc(allocationsCol, String(item.id))));
    Object.values(grouped).forEach(item => { batch.set(doc(allocationsCol), { ...item, createdAt: Date.now() }); });
    discarded.forEach(op => { batch.set(doc(discardedOpsCol), op); });
    await batch.commit();
  };
  ...
  const handleClearAll = async () => {
    ...
    const batch = writeBatch(db);
    allocations.forEach(item => batch.delete(doc(allocationsCol, String(item.id))));
    discardedOps.forEach(item => batch.delete(doc(discardedOpsCol, String(item.id))));
    await batch.commit();
  ...
  const toggleStatus = useCallback(async (item) => {
    ...
    await updateDoc(doc(allocationsCol, String(item.id)), { status: newStatus });
  }, [user]);

  const handleAddPerda = useCallback(async (item) => {
    if (!user || !item.id) return;
    const newQty = (parseFloat(item.quantidade) || 0) + parseFloat(lossValue);
    const newCount = (item.perdaCount || 0) + 1;
    await updateDoc(doc(allocationsCol, String(item.id)), { perdaCount: newCount, quantidade: newQty });
  }, [user, lossValue]);

  const handleResetPerda = useCallback(async (item) => {
    if (!user || !item.id) return;
    const currentCount = item.perdaCount || 0;
    const newQty = (parseFloat(item.quantidade) || 0) - (currentCount * parseFloat(lossValue));
    await updateDoc(doc(allocationsCol, String(item.id)), { perdaCount: 0, quantidade: newQty });
  }, [user, lossValue]);
  ...
  const handleImportExcel = async (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const batch = writeBatch(db);
        rows.filter(row => row.length > 0 && row[idx.maquina] !== undefined).forEach((row, index) => {
          let maq = String(row[idx.maquina] || '').trim(); if (/^\d+$/.test(maq)) maq = `EXT${maq}`;
          batch.set(doc(allocationsCol), { sequencia: '', maquina: maq, item: String(row[idx.item] || ''), itemFinal: String(row[idx.itemFinal] || ''), descricao: String(row[idx.descricao] || ''), quantidade: parseFloat(String(row[idx.quantidade] || 0).replace('.', '').replace(',', '.')) || 0, ordemProducao: String(row[idx.op] || ''), perdaCount: 0, status: 'Pendente', createdAt: Date.now() + index });
        });
        await batch.commit();
  ...
  const handleSave = async (e) => {
    ...
    const dataToSave = { ...formData, quantidade: isNaN(qty) ? 0 : qty, createdAt: editingId ? (allocations.find(i => i.id === editingId)?.createdAt || Date.now()) : Date.now() };
    const docRef = editingId ? doc(allocationsCol, String(editingId)) : doc(allocationsCol);
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-8 flex flex-col gap-6">
        <section className="bg-white p-4 rounded-3xl shadow-sm border border-slate-200">
          <div className="flex flex-col lg:flex-row gap-4 justify-between items-center">
            <div className="relative w-full lg:w-[450px]">
              ...
              <input type="text" placeholder="Pesquisar..." className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-medium" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
            <div className="flex flex-wrap gap-2 w-full lg:w-auto">
              <button onClick={() => { setIsQtyOnlyMode(false); setEditingId(null); setFormData({ sequencia: '', maquina: '', item: '', itemFinal: '', descricao: '', quantidade: '', ordemProducao: '', perdaCount: 0, status: 'Pendente' }); setIsModalOpen(true); }} disabled={allocations.length >= MAX_ROWS} className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white hover:bg-blue-700 rounded-2xl transition-all shadow-lg font-black uppercase text-xs tracking-widest"><Icons.Plus /> Nova Alocação</button>
              <button onClick={() => setIsClearModalOpen(true)} className="flex items-center gap-2 px-6 py-3 bg-red-50 text-red-600 border border-red-100 rounded-2xl font-black uppercase text-xs hover:bg-red-600 hover:text-white transition-all shadow-md"><Icons.Trash /> Limpar</button>
            </div>
          </div>
        ...
        <section className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                      ...
                      <tr className="hover:bg-slate-50 transition-colors group">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button onClick={async () => { if(window.confirm('Eliminar?')) await deleteDoc(doc(allocationsCol, String(item.id))); }} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all"><Icons.Trash /></button>
                          </div>
                        </td>
                      </tr>
                      {isLastOfSequence && (
          ...
          <div className="bg-slate-50 px-6 py-4 border-t border-slate-200 flex justify-between items-center text-[10px] font-black uppercase tracking-widest mt-auto">
            <span>Total: <span className={allocations.length >= MAX_ROWS ? 'text-red-600' : 'text-blue-600'}>{allocations.length} / {MAX_ROWS}</span></span>
            <span className="text-[9px] text-slate-300 italic tracking-tighter uppercase">Sincronizado na nuvem (PCP MG1)</span>
          </div>
        </section>
        ...
        <section className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden max-w-2xl">
          ...
          <div className="max-h-[300px] overflow-y-auto divide-y divide-slate-100">
             {discardedOps.map((op, idx) => <div key={op.id || idx} className="px-6 py-3 font-bold text-slate-700 text-xs flex justify-between"><span>{op.maquina || '-'}</span><span className="font-mono text-slate-500">{op.ordemProducao || '-'}</span></div>)}
          </div>
        </section>
      </main>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-sm overflow-hidden p-8">
            ...
            <div className="space-y-6">
              ...
              <button onClick={() => saveSettings(lossValue)} className="w-full py-4 bg-blue-600 text-white rounded-[20px] font-black text-xs uppercase shadow-xl">Salvar Alteração</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isClearModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-sm p-8 text-center">
            ...
            <p className="text-slate-500 text-sm font-medium mb-8">Esta ação irá apagar os dados para TODOS os utilizadores. Confirmar?</p>
            <div className="flex gap-3"><button onClick={() => setIsClearModalOpen(false)} className="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl font-black text-xs">Cancelar</button><button onClick={handleClearAll} className="flex-1 py-4 bg-red-600 text-white rounded-2xl font-black text-xs shadow-lg">Sim, Limpar</button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto py-20">
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleSave} className="p-8 space-y-6">
              ...
              <div className="pt-4 flex gap-4"><button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-4 border-2 border-slate-100 text-slate-400 rounded-2xl font-black text-xs uppercase">Cancelar</button><button type="submit" className="flex-1 py-4 bg-slate-800 text-white rounded-2xl font-black text-xs shadow-xl">Salvar</button></div>
            </form>
          </div>
        </div>
      )}
