...
const App = () => {
  ...
  const handleSequenciar = async () => {
    ...
    sorted.forEach((item, index, arr) => {
      ...
      const ref = doc(db, 'artifacts', appId, 'public', 'data', 'allocations', String(item.id));
      batch.update(ref, { sequencia: currentSeq });
    });
    await batch.commit();
  };
  ...
  const handleJuncao = async () => {
    ...
    discarded.forEach(op => {
      const newOpRef = doc(discardedOpsCol);
      batch.set(newOpRef, op);
    });
    await batch.commit();
  };
  ...
  const handleClearAll = async () => {
    ...
    const batch = writeBatch(db);
    allocations.forEach(item => batch.delete(doc(db, 'artifacts', appId, 'public', 'data', 'allocations', String(item.id))));
    discardedOps.forEach(item => batch.delete(doc(db, 'artifacts', appId, 'public', 'data', 'discardedOps', String(item.id))));
    await batch.commit();
  ...
  const toggleStatus = async (id, current) => {
    if (!user) return;
    await setDoc(doc(allocationsCol, id), { status: current === 'Pendente' ? 'Conferido' : 'Pendente' }, { merge: true });
  };
                  ...
                  return (
                    <React.Fragment key={item.id}>
                      <tr className="hover:bg-slate-50 transition-colors group">
                        ...
                        <td className="px-6 py-4 text-center">
                          <button onClick={() => toggleStatus(item.id, item.status)} className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${item.status === 'Conferido' ? 'bg-emerald-500 text-white border-emerald-600 shadow-sm' : 'bg-white text-slate-400 border-slate-200'}`}>{item.status === 'Conferido' ? <Icons.Check /> : <Icons.Pending />} {item.status}</button>
                        </td>
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button onClick={async () => { if(window.confirm('Eliminar?')) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'allocations', item.id)); }} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all"><Icons.Trash /></button>
                          </div>
                        </td>
                      </tr>
                      {isLastOfSequence && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto py-20">
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleSave} className="p-8 space-y-6">
              ...
              <div className="pt-4 flex gap-4"><button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-4 border-2 border-slate-100 text-slate-400 rounded-2xl font-black text-xs">Cancelar</button><button type="submit" className="flex-1 py-4 bg-slate-800 text-white rounded-2xl font-black text-xs shadow-xl transition-all">Salvar</button></div>
            </form>
          </div>
        </div>
      )}
