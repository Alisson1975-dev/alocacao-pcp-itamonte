...
// Definição manual de ícones SVG para garantir estabilidade e evitar erros de importação
const Icons = {
  ...
  Import: () => <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg>,
...
const App = () => {
  ...
  const saveSettings = async (newValue) => {
    ...
    try {
      ...
      setCopyFeedback({ type: 'success', message: 'Definição guardada!' });
    } catch (err) {
  ...
  const handleClearAll = () => {
    ...
    setCopyFeedback({ type: 'success', message: 'Sistema limpo.' });
  };
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (allocations.length + newItems.length > MAX_ROWS) {
          setCopyFeedback({ type: 'error', message: 'Limite de 800 excedido!' });
        } else {
  ...
  const handleOpenModal = (item = null, qtyOnly = false) => {
    ...
    if (item) {
      ...
      setFormData({ sequencia: '', maquina: '', item: '', itemFinal: '', descricao: '', quantidade: '', ordemProducao: '', perdaCount: 0, status: 'Pendente' });
      setEditingId(null);
    }
    setIsModalOpen(true);
  };
        ...
        <section className="bg-white p-4 rounded-3xl shadow-sm border border-slate-200">
          <div className="flex flex-col lg:flex-row gap-4 justify-between items-center">
            ...
            <div className="flex flex-wrap gap-2 w-full lg:w-auto">
              <button onClick={() => handleOpenModal()} disabled={allocations.length >= MAX_ROWS} className={`flex-1 lg:flex-none flex items-center justify-center gap-2 px-6 py-3 rounded-2xl transition-all shadow-lg font-black text-xs uppercase tracking-widest ${allocations.length >= MAX_ROWS ? 'bg-slate-200 text-slate-400 shadow-none' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-blue-100'}`}><Icons.Plus /> Nova Alocação</button>
                  ...
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button 
                          onClick={() => handleAddPerda(item.id)}
                          className={`min-w-[64px] py-2 rounded-xl text-xs font-black transition-all border-2 active:scale-95 shadow-sm
                            ${(item.perdaCount || 0) > 0 
                              ? 'bg-emerald-500 text-white border-emerald-600 shadow-md' 
                              : 'bg-white text-slate-300 border-slate-100 hover:border-slate-300 hover:text-slate-500'}`}
                        >
                          {lossValue}
                        </button>
                        {(item.perdaCount || 0) > 0 && (
                          <div className="flex flex-col gap-0.5 items-start">
                            ...
                            <button 
                              onClick={() => handleResetPerda(item.id)}
                              className="text-[8px] font-black text-slate-300 hover:text-red-400 uppercase tracking-tighter flex items-center gap-0.5"
                            >
                              <Icons.Refresh /> reset
                            </button>
                          </div>
      ...
      {isClearModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-sm p-8 text-center">
            ...
            <p className="text-slate-500 text-sm font-medium mb-8">Apagar permanentemente todas as linhas e as OPs descartadas? Esta ação não pode ser desfeita.</p>
